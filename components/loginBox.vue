<template>
  <div class="login">
    ...
    <div class="content">
      <span class="login">ورود | ثبت&zwnj;نام</span>
    </div>
    <div class="hello">
      <p class="login">سلام !</p>
      <p class="login">لطفا شماره موبایل یا ایمیل خود را وارد کنید</p>
    </div>
    <div class="input">
      <Form @submit="onSubmit">
        <Field name="email" type="email" :rules="validateEmail" class="email" />
        <ErrorMessage name="email" class="erorr" />
        <button class="enter">ورود</button>
      </Form>
...
export default {
  ...
  mounted() {
    document.querySelector(".enter").addEventListener("click", this.value);
  },
  beforeDestroy() {
    document.querySelector(".enter").removeEventListener("click", this.value);
  },
  methods: {
    onSubmit(values) {
      console.log(values);
    ...
    validateEmail(value) {
      ...
      if (!regex.test(value) && !phone.test(value)) {
        ...
        return "مقدار این فیلد نامعتبر می‌باشد";
      }
    ...
    value() {
      const valueInput = document.querySelector(".email").value;
      const formData = new FormData();
      formData.append("client_id", "digi");
      formData.append("grant_type", "password");
      formData.append("client_secret", "z69HuHlQLjv03IWSzGOM1TKYRhu53Mgr");
      formData.append("scope", "openid offline_access");
      formData.append("username", valueInput);
      formData.append("password", "test");

      fetch(
        "https://keycloak.asr24.com/realms/test-development/protocol/openid-connect/token",
        {
          method: "POST",
          headers: {
